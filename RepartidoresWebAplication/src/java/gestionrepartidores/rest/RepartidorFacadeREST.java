...
import gestionrepartidores.ejb.RepartidorEJBLocal;
...
@Path("repartidor")
public class RepartidorFacadeREST {

    private static final Logger LOGGER= Logger.getLogger("gestionrepartidores.ejb");
            
    @EJB
    private RepartidorEJBLocal ejb;

    @GET
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public Collection<Repartidor> findAll() {
        Collection<Repartidor> x = null;
        try {
            LOGGER.info("obteniendo repartidores");
            x = ejb.getAllRepartidores();
            
        } catch (ExceptionGetAllRepartidores ex) {
